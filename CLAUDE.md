...
- ✅ **直近で済んだこと**（2026-05-25・先生が詰将棋を作って公開できる管理画面を新設）:
...
- 🟡 **進行中**: なし
- 🔜 **次の一歩（候補）**: 1) 先生の「詰将棋を作る」画面を実機（iPhone・PC）で試して、実際に1問作って公開してみる、2) 詰将棋コーナーに本物の名作（古典・著作権切れ）を入れる＝信頼できる棋譜データの入手方法を決める、3) AIコメント（Phase G）の実装
