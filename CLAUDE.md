...
- ✅ **直近で済んだこと**（2026-05-25 夜・詰将棋を全部やめて削除）:
  - **詰将棋の「あそぶ機能」をすべて削除**（毎日の一問・指で解くモード・先生の問題づくり）。`/tsume`・`/family/tsume` のページ・部品・データ処理を撤去
  - **記録カテゴリ「詰将棋」も削除**。練習を記録するときの種類から「詰将棋」が消えました（棋譜並べ・実戦・研究会など他の種類はそのまま）
  - **本番データベースも整理済み**：詰将棋カテゴリ・それで記録されていた15件・問題テーブル `tsume_problems`・「詰将棋50時間」バッジを削除
  - 生徒/先生メニュー・ダッシュボードの「今日の一問」・トップページの紹介文からも詰将棋を撤去
  - 型チェック・lint・本番ビルドすべて通過／GitHub に push 済（Vercel が自動公開・kido-phi.vercel.app）。ルート一覧から `/tsume`・`/family/tsume` が消えたことを確認
  - ※「名言の間」の詰将棋の格言（「一に詰将棋、二に詰将棋…」等）と棋書本棚の例文は別機能なので残しています
- ✅ **その前**（2026-05-25・先生が詰将棋を作って公開できる管理画面を新設）:
  - **先生メニューに「詰将棋を作る」を追加**（先生だけ表示。`/family/tsume`）
    - 一覧：各問題に「直す・見る」「公開する／下書きにもどす」「消す」ボタン。公開を押すと生徒・保護者の画面に出る
    - **盤エディタ（操作性重視）**：盤をタップして駒を並べ→答えを指で指すと、盤の動き（frames）・正解手順（USI/日本語）が自動でできる（`components/tsume/TsumeEditor.tsx`・`EditBoard.tsx`）
    - **記号(SFEN)貼り付けでも作れる**：開始局面＋USI手順を貼って「確認する」→自動で盤に変換（はじめさん要望）
    - その場で「ためし解き」して確認、むずかしさ・タイトル・出どころ・解説を付けて保存
  - **指し手を盤に反映する仕組みを新規作成**（`lib/shogi-move.ts`：SFEN生成／指し手適用（取る・成る・打つ）／frames生成／日本語表記）。既存9問で動作トレース確認済み
  - **生徒・保護者の詰将棋をデータベース連動に変更**：公開された問題（`kido.tsume_problems`）から表示。1問も公開がなければ従来の組み込み9問にフォールバック（`lib/tsume-fetch.ts`）
  - **DB新設**：migration `0017_tsume_problems.sql`（先生だけ作成/編集/削除・公開済みは全員閲覧のRLS）。**本番Supabaseに適用済み**＋既存9問を初期投入
  - 型チェック・lint・本番ビルドすべて通過（`/family/tsume`・`/family/tsume/new`・`/family/tsume/[id]` がルートに出ることを確認）
  - 🟡 **本番反映：このあと GitHub に push → Vercel 自動公開（kido-phi.vercel.app）**
- ✅ **その前**（2026-05-25・詰将棋を「指で動かして解ける」ように＋出どころ表示）:
...
- 🟡 **進行中**: なし
- 🔜 **次の一歩（候補）**: 1) 実機（iPhone・PC）で詰将棋が消えて他の画面が普通に動くか確認、2) 通知（Vercel 環境変数の登録）の動作確認、3) AIコメント（Phase G）の実装
