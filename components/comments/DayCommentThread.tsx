...
import {
  addDayCommentAction,
  deleteDayCommentAction,
  updateDayCommentAction
} from '@/app/_actions/day-comments'

interface Props {
  studentId: string
  date: string // YYYY-MM-DD
  comments: DayCommentItemView[]
  /** 書き込み可能か */
  canPost: boolean
  /** プレースホルダ（ロール別に差し替え） */
  placeholder?: string
...
export function DayCommentThread({
  studentId,
  date,
  comments,
  canPost,
  placeholder = 'この日への声かけ・励まし・コメントを書こう'
}: Props) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    startTransition(async () => {
      const result = await addDayCommentAction({ studentId, date, content: trimmed })
      if (!result.ok) {
  ...
  const handleDelete = (id: string) => {
    ...
    setError(null)
    startTransition(async () => {
      const result = await deleteDayCommentAction({ commentId: id })
      if (!result.ok) {
        setError(result.error)
        return
      }
  ...
  const handleStartEdit = (c: DayCommentItemView) => {
    setEditingId(c.id)
  ...
  const handleSaveEdit = (id: string) => {
    ...
    setError(null)
    startTransition(async () => {
      const result = await updateDayCommentAction({ commentId: id, content: trimmed })
      if (!result.ok) {
        setError(result.error)
        return
      }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between text-xs text-text-muted px-1">
        <span className="font-semibold">この日への声かけ</span>
        <span className="text-[10px] text-text-dim">
          {comments.length === 0 ? '記録がなくても書けます' : `${comments.length}件`}
        </span>
      </div>
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="ml-auto text-[10px] text-text-dim">
                      ...
                      {edited && <span className="ml-1 text-text-dim">（編集済み）</span>}
                    </span>
                  </div>
      ...
      {canPost ? (
        <form onSubmit={handleSubmit} className="flex flex-col gap-2 mt-1">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-text-dim font-num">{content.length}/2000</span>
            <button
              type="submit"
        ...
        comments.length === 0 && (
          <p className="text-xs text-text-dim text-center py-2">まだコメントはありません</p>
        )
      )}
    </div>
  )
